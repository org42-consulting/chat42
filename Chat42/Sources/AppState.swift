...
final class AppState {
    ...
    func refreshGatewayModels() async {
        ...
        do {
            ...
            if selectedGatewayModel == nil || !models.contains(where: { $0.id == selectedGatewayModel?.id }) {
    ...
    func refreshOllamaModels(reportError: Bool = false) async {
        ...
        do {
            ...
            ollamaModels = models
            if selectedOllamaModel == nil || !models.contains(where: { $0.name == selectedOllamaModel?.name }) {
    ...
    func sendMessage(_ text: String) async {
        ...
        do {
            let contextMessages = conversation.messages.filter { $0.role != .assistant || $0 !== assistantMessage }
            ...
            assistantMessage.content = String(format: String(localized: "error.generic"), error.localizedDescription)
        }
    }
